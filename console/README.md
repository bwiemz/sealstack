...
The administrative web interface for ContextForge.
...
The console expects a ContextForge gateway on `http://localhost:7070` by
default. Override via the Settings page or by setting
`PUBLIC_GATEWAY_URL` in `.env`.

With the gateway running (`cfg dev` in the workspace root), the full e2e loop
works:

1. Open `http://localhost:7071`
2. Settings → verify the gateway is reachable
3. Schemas → confirm `examples.Doc` is registered (run `cfg schema apply`
   first if not)
