...
description: How SealStack scopes queries so tenants never see each other's data.
---
...
- **MCP routes** — the `tenant` claim on the validated JWT, courtesy of the
  OAuth bearer middleware at [`src/auth.rs`](https://github.com/bwiemz/sealstack/blob/main/crates/sealstack-gateway/src/auth.rs).
