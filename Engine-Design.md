...
**Companion to**: `crates/cfg-engine/` in the drop-in.
...
### Backends
- **Vector**: `cfg_vectorstore::VectorStore::search`. In dev, `InMemoryStore`; in production, `QdrantStore`.
...
Input: `cfg_connector_sdk::Resource { id, kind, title, body, metadata, permissions, source_updated_at }`.
...
Receipts persist in `cfg_receipts` with indexes on `caller_id`, `created_at`, and `qualified_schema`. Retention is 90 days by default; a `ReceiptStore::prune` method is provided for the nightly cleanup cron.
...
1. **Engine control-plane tables** (`cfg_schemas`, `cfg_connectors`, `cfg_receipts`, `cfg_ingest_state`, `cfg_lineage`, `cfg_mcp_sessions`) — live in `migrations/` and run via `sqlx::migrate!` on startup.
2. **Per-CSL-schema tables** (`customer`, `ticket`, `customer_chunk`, etc.) — emitted by `cfg_csl::codegen::sql` into the compile output directory and applied via `Store::apply_schema_ddl` when `cfg schema apply` runs.
...
7. **Hot reload.** The registry is immutable after `Engine::new`. A file-watcher in `SchemaRegistry` plus a read-write lock lets `cfg schema apply` take effect without restart.
...
7. **Binary `cfg-gateway`.** It now imports `cfg_vectorstore::memory::InMemoryStore`, `cfg_vectorstore::qdrant::QdrantStore`, `cfg_embedders::stub::StubEmbedder`. If those paths differ in the actual crates, the binary won't link — fix the paths.
...
use cfg_vectorstore::memory::InMemoryStore;
use cfg_embedders::stub::StubEmbedder;
...
async fn main() -> anyhow::Result<()> {
    ...
    let caller = cfg_engine::api::Caller::test("u_dev");
...
The `cfg-gateway` binary does exactly this composition (in `bin/server.rs`) and then wraps the `Engine` as `Arc<dyn EngineFacade>` before handing it to `build_app`.
