...
const UA: &str = concat!("contextforge-github/", env!("CARGO_PKG_VERSION"));
...
pub struct GithubConfig {
    ...
    pub token: String,
    /// The user or org login to pull from (e.g. `"contextforge"`). When
...
impl GithubConfig {
    ...
    pub fn from_json(v: &serde_json::Value) -> CfgResult<Self> {
        let token = v
            ...
            .ok_or_else(|| {
                CfgError::Config("github connector requires `token` or GITHUB_TOKEN env".into())
            })?;
        let owner = v
            .get("owner")
            .and_then(|x| x.as_str())
...
impl GithubConnector {
    ...
    async fn get_json<T: for<'de> Deserialize<'de>>(&self, url: &str) -> CfgResult<(T, Option<String>)> {
        let resp = self
            .client
            .get(url)
            ...
            .send()
            .await
            .map_err(|e| CfgError::Backend(format!("github request: {e}")))?;

        if resp.status() == reqwest::StatusCode::UNAUTHORIZED {
            return Err(CfgError::Unauthorized("github token rejected".into()));
        }
        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            return Err(CfgError::Backend(format!(
                "github {status}: {}",
        ...
        let value = resp
            .json::<T>()
            .await
            .map_err(|e| CfgError::Backend(format!("github decode: {e}")))?;
        Ok((value, next))
    }

    async fn list_repos(&self) -> CfgResult<Vec<Repo>> {
    ...
    async fn fetch_readme(&self, owner: &str, repo: &str) -> CfgResult<Option<String>> {
        ...
        match self.get_json::<ReadmeBody>(&url).await {
            ...
            Err(CfgError::Backend(m)) if m.contains("404") => Ok(None),
            Err(e) => Err(e),
        }
    }

    async fn list_issues(&self, owner: &str, repo: &str) -> CfgResult<Vec<Issue>> {
        let mut url =
