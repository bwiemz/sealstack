...
**Companion to**: `crates/cfg-cli/` in the drop-in, plus the updated `crates/cfg-gateway/src/{rest,server,bin/server}.rs`.
**Status**: Unverified skeleton. 1,539 LOC for the CLI, ~300 LOC of gateway REST handlers, plus a small patch to `cfg-csl` that emits schema metadata for the CLI to forward.
...
1. **A thin HTTP client** over the gateway's REST surface. `cfg schema list`, `cfg connector sync`, `cfg query`, `cfg receipt` — all of them just hit an endpoint and render the response.
2. **A local CSL compiler driver.** `cfg compile` and `cfg schema validate` run `cfg_csl::compile()` in-process without touching the gateway. `cfg schema apply` is the hybrid — compile locally, then POST the results.
3. **A Docker Compose wrapper.** `cfg dev` shells out to `docker compose up -d` against a well-known file path, then polls `/healthz` until ready.
...
```
  cfg <subcommand> ────▶ cli::Cli::parse
                       ...
                       │             │
                       ▼             ▼
                 client::Client   cfg_csl::compile  (+ filesystem I/O)
                 (HTTP client)
                       │
                       ▼
...
# 2. Boot the stack
cfg dev              # docker compose up -d + poll /healthz
...
# 5. Query
cfg query "getting started" --schema examples.Doc
cfg receipt <receipt_id>
```
...
`cfg schema apply <path>` is the only command that splits work between the CLI and the gateway:

1. CLI reads the `.csl` file from disk.
2. CLI calls `cfg_csl::compile(src, CompileTargets::all())`.
...
Two round trips, not one. The split matters because the DDL is persisted in Postgres and survives a gateway restart, while the in-memory registry does not — the next restart will re-hydrate from `cfg_schemas` if we wire that up, but today the operator re-runs `cfg schema apply` on restart.
...
To make `schema apply` work, I patched `cfg-csl` to emit a `schemas_meta: Vec<Value>` field on `CompileOutput`. Each entry is a JSON document matching `cfg_engine::schema_registry::SchemaMeta` — namespace, name, version, primary key, fields with decorator flags (`primary`, `indexed`, `searchable`, `chunked`, `facet`, `unique`, `optional`, plus `boost` and `pii` values), relations, context block, collection name (`<table>_v<version>`), table name (snake_cased schema name), and optional schema-level hybrid alpha.
...
**Caveat:** the emitter makes strong assumptions about the `cfg-csl` AST shape — specifically `FieldDecl.decorators`, `Decorator::is()`, `Arg.name`, `Expr::Call`, `Literal::Duration`, and `rel.kind.as_str()`. If any of those differ from what my AST module declares, the patch will fail to compile. The fix is local and mechanical — adjust the field accesses to match the actual AST.
...
The gateway's `CallerExt` extractor picks these up and constructs a `cfg_engine::api::Caller` that flows through the engine's policy layer.
...
5. **`cfg-csl` emits `schemas_meta`** on `CompileOutput`. See §4 above.

---

## 8. Known gaps — priority order

1. **Compile-error rendering.** `cfg schema validate` currently prints `CslError::Display`. The CSL spec calls for `miette` rendering with spans, underlines, and help text. Adding `miette` as a CLI dep and a one-liner `Report::new(err).with_source_code(src)` gives the full experience.

2. **Real auth.** Header-based identity is a dev convenience. Wire in JWT validation in a Tower middleware layer ahead of `CallerExt`. Operators get an `OAUTH_TOKEN` or `CFG_TOKEN` env var; the CLI sends it as a bearer. Target: next sprint.

3. **Receipt retrieval latency.** `cfg receipt <id>` hits `/v1/receipts/<id>`, which does a single `SELECT` against `cfg_receipts`. Fast, but there's no caching. Not a problem yet.

4. **No interactive confirmation.** `cfg schema apply` on an existing schema silently overwrites the in-memory registry entry. A real production flow should prompt unless `--yes` is passed.

5. **`cfg watch` is missing.** The local-files connector already supports `subscribe`. The CLI should expose `cfg connector watch <id>` that spawns a long-running session tailing change events. ~50 LOC.

6. **Background connector scheduling.** `cfg connector add` registers a binding with `interval = None`; the gateway's `IngestRuntime::start_background` is never called. For operational deployments we want a `--interval 5m` flag plus a `cfg connector schedule` command.

7. **`cfg dev --tail-logs`.** After `docker compose up -d`, many operators immediately run `docker compose logs -f`. A `--tail-logs` flag should just do that inline.
...
10. **Rate-limit the `healthz` poll in `cfg dev`.** Currently 500 ms. Should back off exponentially to avoid log spam when the gateway takes 30+ seconds to come up on a fresh docker build.
...
6. **clap `env = "CFG_GATEWAY_URL"` + `default_value = "..."`.** Stable since clap 4.0.

7. **`toml` 0.8 deserializer.** Works through `toml::from_str`. Stable.

If `cargo check -p cfg-cli` passes with the CLI alone — which doesn't depend on `cfg-engine`, `cfg-gateway`, or the connector SDK — the bulk of the code is valid. The gateway-side patches are the higher-risk surface.
...
cfg connector add local-files --schema examples.Doc --root ./sample-docs
cfg connector sync local-files/examples.Doc
result=$(cfg query "getting started" --schema examples.Doc --json)
echo "$result" | jq -e '.results | length > 0'
receipt=$(echo "$result" | jq -r .receipt_id)
cfg receipt "$receipt" --json | jq -e '.sources | length > 0'
```
