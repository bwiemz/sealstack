...
//! # let vector_store: Arc<dyn signet_vectorstore::VectorStore> = todo!();
//! # let embedder:     Arc<dyn signet_embedders::Embedder>     = todo!();
//!
//! let engine = Engine::new_dev(config, vector_store, embedder).await?;
//! let engine: Arc<dyn signet_engine::EngineHandle> = Arc::new(engine);
//!
...
impl Engine {
    ...
    /// Access the ingestor (used by `signet-ingest`).
    #[must_use]
