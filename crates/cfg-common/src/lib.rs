//! Shared types, errors, and identifiers used across every ContextForge crate.
//!
...
/// converts into `cfg_engine::api::EngineError` at its surface; the gateway
/// converts into `cfg_gateway::mcp::ToolError`. Each layer gets its own error
...
mod tests {
    ...
    fn cfg_error_display() {
        let e = CfgError::Backend("boom".into());
