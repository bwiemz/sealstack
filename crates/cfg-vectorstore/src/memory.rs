...
impl InMemoryStore {
    ...
    fn ensure(&self, name: &str, dims: usize, distance: Distance) -> CfgResult<()> {
...
impl VectorStore for InMemoryStore {
    ...
    async fn upsert(&self, collection: &str, chunks: Vec<Chunk>) -> CfgResult<()> {
        let mut entry = self.collections.get_mut(collection).ok_or_else(|| {
            CfgError::NotFound(format!("collection `{collection}` not found"))
        })?;
...
mod tests {
    ...
    async fn dims_mismatch_is_validation_error() {
        ...
        assert!(matches!(err, CfgError::Validation(_)));
    }
