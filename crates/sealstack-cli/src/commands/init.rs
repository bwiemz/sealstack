//! `sealstack init` — scaffold a new SealStack project.
...
pub(crate) async fn run(ctx: &CliContext, args: InitArgs) -> anyhow::Result<()> {
    ...
    output::print(
        ...
        &json!({
            ...
            "sealstack_toml": toml_path.display().to_string(),
...
const STARTER_CSL: &str = r#"// Starter schema. Run `sealstack schema apply` to register it with the gateway.
...
SealStack stands up in three commands once you have Docker running:
