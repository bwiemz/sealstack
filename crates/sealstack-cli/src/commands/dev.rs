//! `sealstack dev` — boot / teardown the local Docker Compose stack.
...
/// 3. `SEALSTACK_HOME/compose.dev.yaml` if `$SEALSTACK_HOME` is set.
fn resolve_compose_file(explicit: Option<&Path>, project_root: &Path) -> anyhow::Result<PathBuf> {
    ...
    if let Ok(home) = std::env::var("SEALSTACK_HOME") {
    ...
    bail!(
        "could not find a compose file; searched `deploy/docker/compose.dev.yaml` and $SEALSTACK_HOME. \
