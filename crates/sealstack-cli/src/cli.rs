...
#[command(
    name = "cfg",
    version,
    about = "SealStack command-line interface",
    long_about = None,
)]
pub(crate) struct Cli {
    /// Gateway base URL. Defaults to `$SEALSTACK_GATEWAY_URL` or `http://localhost:7070`.
    #[arg(
        long,
        global = true,
        env = "SEALSTACK_GATEWAY_URL",
        default_value = "http://localhost:7070"
    )]
    pub gateway_url: String,

    /// Caller user id sent via `X-Cfg-User`. Defaults to `$USER` or `"anon"`.
    #[arg(long, global = true, env = "SEALSTACK_USER")]
...
pub(crate) enum Command {
    ...
    /// Scaffold a SealStack project in the current directory.
    Init(InitArgs),
...
pub(crate) struct ConnectorSyncArgs {
    /// Binding id (from `sealstack connector list`), e.g. `"local-files/examples.Doc"`.
    pub id: String,
}
