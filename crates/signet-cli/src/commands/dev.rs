//! `signet dev` — boot / teardown the local Docker Compose stack.
...
/// 3. `SIGNET_HOME/compose.dev.yaml` if `$SIGNET_HOME` is set.
fn resolve_compose_file(explicit: Option<&Path>, project_root: &Path) -> anyhow::Result<PathBuf> {
    ...
    bail!(
        "could not find a compose file; searched `deploy/docker/compose.dev.yaml` and $SIGNET_HOME. \
