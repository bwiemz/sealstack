...
use crate::{SignetError, SignetResult, Embedder};
...
impl OpenAIEmbedder {
    ...
    pub fn new(api_key: impl Into<String>) -> SignetResult<Self> {
        Self::with_model(api_key, "text-embedding-3-small")
    }

    /// Construct with a specific model.
    pub fn with_model(api_key: impl Into<String>, model: impl Into<String>) -> SignetResult<Self> {
        ...
        let api_key = {
            ...
            if provided.is_empty() {
                std::env::var("OPENAI_API_KEY")
                    .map_err(|_| SignetError::Config("missing OPENAI_API_KEY".into()))?
            } else {
                provided
            }
        };
...
impl Embedder for OpenAIEmbedder {
    ...
    async fn embed(&self, texts: Vec<String>) -> SignetResult<Vec<Vec<f32>>> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(SignetError::backend)?;

        let status = resp.status();
        if status == reqwest::StatusCode::TOO_MANY_REQUESTS {
            return Err(SignetError::RateLimited);
        }
        if !status.is_success() {
            let body = resp.text().await.unwrap_or_default();
            return Err(SignetError::Backend(format!("openai {status}: {body}")));
        }

        let parsed: EmbedResp = resp.json().await.map_err(SignetError::backend)?;
