...
//! corresponding method on [`cfg_engine::facade::EngineFacade`].
//!
//! The facade trait lives in `cfg-engine` so the engine crate can provide the
//! single blanket impl (any `EngineHandle` becomes an `EngineFacade` via the
//! blanket in `cfg_engine::facade`). The gateway does not know about the
//! concrete engine type — only the trait object.
//!
//! Handler responsibilities:
//! 1. Validate arguments against the CSL-generated JSON Schema.
//! 2. Convert the gateway's [`Caller`] into [`cfg_engine::api::Caller`].
...
use cfg_engine::api::EngineError;
...
pub struct GeneratedHandler {
    /// The pre-computed descriptor (from `cfg_csl::codegen::mcp`).
...
// cfg-engine's type namespace.
// ---------------------------------------------------------------------------

fn caller_to_engine(c: &Caller) -> cfg_engine::api::Caller {
    cfg_engine::api::Caller {
        id: c.id.clone(),
...
// Replaced with `cfg_engine::Engine` in production. Feature-gated so it does
