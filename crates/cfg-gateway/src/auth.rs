...
impl AuthMode {
    ...
    ///
    /// * `CFG_AUTH_MODE=disabled|hs256`
    /// * `CFG_AUTH_HS256_SECRET=<bytes>` (required when `hs256`)
    /// * `CFG_AUTH_ISSUERS=iss1,iss2` (optional)
    /// * `CFG_AUTH_AUDIENCES=aud1,aud2` (optional)
    #[must_use]
    pub fn from_env() -> Self {
        let mode = std::env::var("CFG_AUTH_MODE").unwrap_or_else(|_| "disabled".into());
        match mode.as_str() {
            "hs256" => {
                let secret = std::env::var("CFG_AUTH_HS256_SECRET").unwrap_or_default();
                if secret.is_empty() {
                    tracing::warn!(
                        "CFG_AUTH_MODE=hs256 but CFG_AUTH_HS256_SECRET is unset; falling back to disabled",
                    );
                    return Self::Disabled;
                }
                let issuers = split_csv("CFG_AUTH_ISSUERS");
                let audiences = split_csv("CFG_AUTH_AUDIENCES");
                Self::Hs256 {
                    secret,
                    issuers,
                    audiences,
...
fn unauthorized(reason: &str) -> Response {
    ...
    if let Ok(v) = HeaderValue::from_str(
        "Bearer realm=\"contextforge\", error=\"invalid_token\", \
