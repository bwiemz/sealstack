...
pub struct Config {
    ...
    /// Default log filter (e.g., `info,cfg_gateway=debug`).
...
impl Config {
    ...
    pub fn from_env() -> Self {
        Self {
            bind: std::env::var("CFG_BIND").unwrap_or_else(|_| "0.0.0.0:7070".into()),
            database_url: std::env::var("CFG_DATABASE_URL")
                .unwrap_or_else(|_| "postgres://cfg:cfg@localhost:5432/cfg".into()),
            qdrant_url: std::env::var("CFG_QDRANT_URL")
                .unwrap_or_else(|_| "http://localhost:6334".into()),
            redis_url: std::env::var("CFG_REDIS_URL").ok(),
