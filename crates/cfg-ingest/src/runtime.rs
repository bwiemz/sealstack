...
//!    (`cfg connector sync <id>`) and by integration tests. Returns a
...
use cfg_connector_sdk::ChangeEvent;
use cfg_engine::{Engine, api::EngineError};
...
impl IngestRuntime {
    ...
    async fn run_sync(
        ...
        let meta = self
            ...
            .map_err(|e| match e {
                EngineError::UnknownSchema { namespace, schema } => CfgError::Config(format!(
                    "target schema `{namespace}.{schema}` is not registered with the engine"
                )),
                other => CfgError::backend(other),
            })?;
    ...
    async fn subscribe_loop(
        &self,
        binding: ConnectorBinding,
        mut stream: cfg_connector_sdk::ChangeStream,
    ) {
        ...
        while let Some(event) = stream.next().await {
            match event {
                ...
                ChangeEvent::Delete(_id) => {
                    ...
                    // via cfg_lineage; plumb that when the lineage queries land.
...
mod tests {
    ...
    use cfg_connector_sdk::{Connector, Resource, ResourceStream, change_streams};
    ...
    impl Connector for EmptyConnector {
        ...
        async fn fetch(&self, _id: &cfg_connector_sdk::ResourceId) -> CfgResult<Resource> {
            Err(CfgError::NotFound("no such resource".into()))
        }
    }
