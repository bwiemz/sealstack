...
impl ReceiptStore {
    ...
    pub async fn persist(&self, receipt: &Receipt) -> Result<String, EngineError> {
        ...
        sqlx::query(
            "INSERT INTO cfg_receipts (id, caller_id, qualified_schema, tool, body, created_at) \
             VALUES ($1, $2, $3, $4, $5, $6)",
        )
        .bind(&receipt.id)
    ...
    pub async fn fetch(&self, id: &str) -> Result<Receipt, EngineError> {
        let row: Option<(Value,)> =
            sqlx::query_as("SELECT body FROM cfg_receipts WHERE id = $1")
                .bind(id)
    ...
    pub async fn prune(&self, retention_days: u32) -> Result<u64, EngineError> {
        ...
        let res = sqlx::query("DELETE FROM cfg_receipts WHERE created_at < $1")
            .bind(cutoff)
            .execute(self.store.pool())
            .await
            .map_err(EngineError::backend)?;
