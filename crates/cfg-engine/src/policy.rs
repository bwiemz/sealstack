...
//! (`cfg_csl::codegen::policy`, not yet emitted). At runtime, this module loads
...
/// the WASM policy codegen lands in `cfg-csl`. The [`Engine::new_dev`] constructor
...
/// * `cfg_alloc(n_bytes: i32) -> i32` — bump/linear allocator returning a
///   linear-memory offset the host may write `n_bytes` into.
/// * `cfg_evaluate(input_ptr: i32, input_len: i32) -> i32` — takes a UTF-8
...
impl WasmPolicyHandle {
    fn call(&self, payload: &[u8]) -> Result<i32, EngineError> {
        ...
        let alloc = instance
            .get_typed_func::<i32, i32>(&mut store, "cfg_alloc")
            .map_err(|e| EngineError::Backend(format!("wasm alloc: {e}")))?;
        let evaluate = instance
            .get_typed_func::<(i32, i32), i32>(&mut store, "cfg_evaluate")
...
mod wasm_tests {
    ...
    const ALWAYS_ALLOW: &str = r#"
        (module
          ...
          (func (export "cfg_alloc") (param $n i32) (result i32)
            (local $p i32)
            global.get $bump
            local.tee $p
            local.get $n
            ...
            global.set $bump
            local.get $p)
          (func (export "cfg_evaluate") (param $in_ptr i32) (param $in_len i32) (result i32)
            i32.const 1))
    "#;
