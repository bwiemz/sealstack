...
/// Persisted-form view of one row in `cfg_connectors`. The engine stores only
...
impl Store {
    ...
    pub async fn put_schema(&self, meta: &SchemaMeta) -> Result<(), EngineError> {
        ...
        sqlx::query(
            "INSERT INTO cfg_schemas (namespace, name, version, definition) \
             VALUES ($1, $2, $3, $4) \
    ...
    pub async fn delete_connector(&self, id: &str) -> Result<bool, EngineError> {
        let res = sqlx::query("DELETE FROM cfg_connectors WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await
            .map_err(EngineError::backend)?;
    ...
    /// Apply a per-schema DDL bundle produced by `cfg_csl::codegen::sql`.
    ///
