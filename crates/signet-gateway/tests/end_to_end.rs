//! End-to-end integration test for the Signet gateway.
//!
...
//! # Why `#[ignore]`
//!
//! The test boots a real [`signet_engine::Engine`], which requires Postgres and
...
//!     cargo test -p signet-gateway --test end_to_end -- --ignored
//! ```
//!
...
fn pg_url() -> Option<String> {
    std::env::var("SIGNET_DATABASE_URL").ok()
}

async fn build_test_app() -> anyhow::Result<(Router, TempDir)> {
    let database_url = pg_url().expect("SIGNET_DATABASE_URL must be set for ignored integration tests");
    let config = signet_gateway::config::Config {
        bind: "127.0.0.1:0".into(),
        database_url: database_url.clone(),
        qdrant_url: String::new(),
        redis_url: None,
        oauth: signet_gateway::mcp::oauth::OAuthMetadataConfig::dev_default(),
        log_filter: "info".into(),
        auth: signet_gateway::auth::AuthMode::Disabled,
    };
    ...
    let app = signet_gateway::build_app(config, engine, factory).await?;
    Ok((app, tmp))
}
...
#[tokio::test]
#[ignore = "requires SIGNET_DATABASE_URL + running Postgres"]
async fn register_schema_then_connector_then_sync() {
    if pg_url().is_none() {
        eprintln!("skipping: SIGNET_DATABASE_URL not set");
        return;
    }
