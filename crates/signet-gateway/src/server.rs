...
//!
//! * `engine` — the concrete [`signet_engine::Engine`]. REST handlers call through
...
use signet_engine::Engine;
use signet_engine::facade::EngineFacade;
use signet_engine::store::PersistedConnector;
use signet_ingest::{ConnectorBinding, ConnectorRegistry, IngestRuntime, SyncOutcome};
...
impl AppState {
    ...
    #[must_use]
    pub fn ingest_bindings(&self) -> Vec<signet_ingest::registry::ConnectorBindingInfo> {
...
/// Reinstantiate every connector binding stored in `signet_connectors`.
///
