...
use crate::{CfgError, CfgResult, Embedder};
...
impl OpenAIEmbedder {
    ...
    pub fn new(api_key: impl Into<String>) -> CfgResult<Self> {
        Self::with_model(api_key, "text-embedding-3-small")
    }

    /// Construct with a specific model.
    pub fn with_model(api_key: impl Into<String>, model: impl Into<String>) -> CfgResult<Self> {
        ...
        let api_key = {
            ...
            if provided.is_empty() {
                std::env::var("OPENAI_API_KEY")
                    .map_err(|_| CfgError::Config("missing OPENAI_API_KEY".into()))?
            } else {
                provided
            }
        };
...
impl Embedder for OpenAIEmbedder {
    ...
    async fn embed(&self, texts: Vec<String>) -> CfgResult<Vec<Vec<f32>>> {
        ...
        if status == reqwest::StatusCode::TOO_MANY_REQUESTS {
            return Err(CfgError::RateLimited);
        }
        if !status.is_success() {
            let body = resp.text().await.unwrap_or_default();
            return Err(CfgError::Backend(format!("openai {status}: {body}")));
        }

        let parsed: EmbedResp = resp.json().await.map_err(CfgError::backend)?;
