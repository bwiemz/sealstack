...
//! # let vector_store: Arc<dyn sealstack_vectorstore::VectorStore> = todo!();
//! # let embedder:     Arc<dyn sealstack_embedders::Embedder>     = todo!();
//!
//! let engine = Engine::new_dev(config, vector_store, embedder).await?;
//! let engine: Arc<dyn sealstack_engine::EngineHandle> = Arc::new(engine);
//!
...
impl Engine {
    ...
    /// Access the ingestor (used by `sealstack-ingest`).
    #[must_use]
