...
//! [`signet_engine::Engine`].
//!
//! # Three modes
//!
//! 1. **One-shot sync** — `sync_once(binding_id)`. Called by the CLI
//!    (`signet connector sync <id>`) and by integration tests. Returns a
...
use signet_connector_sdk::ChangeEvent;
use signet_engine::{Engine, api::EngineError};
...
impl IngestRuntime {
    ...
    async fn run_sync(
        ...
        let meta = self
            ...
            .map_err(|e| match e {
                EngineError::UnknownSchema { namespace, schema } => SignetError::Config(format!(
                    "target schema `{namespace}.{schema}` is not registered with the engine"
                )),
                other => SignetError::backend(other),
            })?;
    ...
    async fn subscribe_loop(
        &self,
        binding: ConnectorBinding,
        mut stream: signet_connector_sdk::ChangeStream,
    ) {
        ...
        while let Some(event) = stream.next().await {
            match event {
                ...
                ChangeEvent::Delete(_id) => {
                    ...
                    // via signet_lineage; plumb that when the lineage queries land.
...
mod tests {
    ...
    use signet_connector_sdk::{Connector, Resource, ResourceStream, change_streams};
    ...
    impl Connector for EmptyConnector {
        ...
        async fn fetch(&self, _id: &signet_connector_sdk::ResourceId) -> SignetResult<Resource> {
            Err(SignetError::NotFound("no such resource".into()))
        }
    }
