//! `cfg init` — scaffold a new ContextForge project.
...
pub(crate) async fn run(ctx: &CliContext, args: InitArgs) -> anyhow::Result<()> {
    ...
    let name = root
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("contextforge-project")
        .to_owned();
    ...
    output::print(
        ...
        &json!({
            ...
            "cfg_toml": toml_path.display().to_string(),
...
const STARTER_CSL: &str = r#"// Starter schema. Run `cfg schema apply` to register it with the gateway.
...
ContextForge stands up in three commands once you have Docker running:
