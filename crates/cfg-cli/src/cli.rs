...
#[command(
    name = "cfg",
    version,
    about = "ContextForge command-line interface",
    long_about = None,
)]
pub(crate) struct Cli {
    /// Gateway base URL. Defaults to `$CFG_GATEWAY_URL` or `http://localhost:7070`.
    #[arg(
        long,
        global = true,
        env = "CFG_GATEWAY_URL",
    ...
    #[arg(long, global = true, env = "CFG_USER")]
...
pub(crate) enum Command {
    ...
    /// Scaffold a ContextForge project in the current directory.
    Init(InitArgs),
...
pub(crate) struct ConnectorSyncArgs {
    /// Binding id (from `cfg connector list`), e.g. `"local-files/examples.Doc"`.
    pub id: String,
}
