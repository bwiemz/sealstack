...
//! (CSL compilation, `cfg dev` docker shelling).
//!
//! # Most commands talk to the gateway
//!
//! | Command                          | Gateway call                              |
//! |----------------------------------|-------------------------------------------|
//! | `cfg schema apply <path>`        | compile locally + `POST /v1/schemas` + `POST /v1/schemas/:q/ddl` |
//! | `cfg schema list`                | `GET /v1/schemas`                         |
//! | `cfg connector add <kind>`       | `POST /v1/connectors`                     |
//! | `cfg connector list`             | `GET /v1/connectors`                      |
//! | `cfg connector sync <id>`        | `POST /v1/connectors/:id/sync`            |
//! | `cfg query <text>`               | `POST /v1/query`                          |
//! | `cfg receipt <id>`               | `GET /v1/receipts/:id`                    |
//!
//! # Local-only commands
//!
//! | Command           | Does                                                       |
//! |-------------------|------------------------------------------------------------|
//! | `cfg compile`     | Compiles every `.csl` file under `./schemas`               |
//! | `cfg init`        | Scaffolds a `cfg.toml` + `schemas/` directory              |
//! | `cfg dev`         | `docker compose -f <compose_file> up -d` + waits for ready |
//! | `cfg version`     | Prints the CLI version                                     |
