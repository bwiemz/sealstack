...
//! corresponding method on [`sealstack_engine::facade::EngineFacade`].
//!
//! The facade trait lives in `sealstack-engine` so the engine crate can provide the
//! single blanket impl (any `EngineHandle` becomes an `EngineFacade` via the
//! blanket in `sealstack_engine::facade`). The gateway does not know about the
//! concrete engine type — only the trait object.
//!
//! Handler responsibilities:
//! 1. Validate arguments against the CSL-generated JSON Schema.
//! 2. Convert the gateway's [`Caller`] into [`sealstack_engine::api::Caller`].
...
pub struct GeneratedHandler {
    /// The pre-computed descriptor (from `sealstack_csl::codegen::mcp`).
...
// sealstack-engine's type namespace.
// ---------------------------------------------------------------------------

fn caller_to_engine(c: &Caller) -> sealstack_engine::api::Caller {
    sealstack_engine::api::Caller {
        id: c.id.clone(),
...
// Replaced with `sealstack_engine::Engine` in production. Feature-gated so it does
...
impl EngineFacade for StubEngineFacade {
    async fn search(
        &self,
        _caller: &sealstack_engine::api::Caller,
        _schema: &str,
        query: &str,
        _top_k: usize,
        _filters: &Value,
