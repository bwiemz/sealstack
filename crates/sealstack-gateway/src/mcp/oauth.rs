...
//!   bounce through.
//!
//! * `/.well-known/oauth-authorization-server` — (optional) if SealStack is
...
impl OAuthMetadataConfig {
    ...
    pub fn dev_default() -> Self {
        let base = std::env::var("SEALSTACK_PUBLIC_URL")
            .unwrap_or_else(|_| "http://localhost:7070".into());
        let auth_server = std::env::var("SEALSTACK_OAUTH_ISSUER")
            .unwrap_or_else(|_| "http://localhost:8080/realms/sealstack".into());
        Self {
            resource: base,
...
async fn handle_protected_resource(
    ...
    Json(ProtectedResourceMetadata {
        ...
        resource_documentation: Some("https://docs.sealstack.ai/mcp/auth".to_owned()),
    })
}
