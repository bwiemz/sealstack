//! Connector SDK.
//!
//! To add a new data source to Signet, create a crate, depend on this
...
//! use signet_connector_sdk::{Connector, Resource, ResourceId, ResourceStream, change_streams};
...
//!     }
//!
//!     async fn fetch(&self, _id: &ResourceId) -> SignetResult<Resource> {
...
pub use signet_common::{SignetError, SignetResult};
...
///
/// The engine's [`Ingestor`](../../signet_engine/ingest/index.html) takes a
/// `Resource` plus a [`SchemaMeta`](../../signet_engine/schema_registry/struct.SchemaMeta.html),
...
///
/// Errors fall into [`SignetError::Backend`] for transient / infrastructural
/// failures and [`SignetError::Unauthorized`] when the source rejects credentials.
/// Never leak source-side PII through error strings.
#[async_trait]
pub trait Connector: Send + Sync + 'static {
    /// Short, stable identifier. Must match the name in
    /// `signet connector add <name>`.
    fn name(&self) -> &str;
    ...
    /// and for the `signet connector refresh <id>` CLI.
