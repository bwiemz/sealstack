...
impl InMemoryStore {
    ...
    fn ensure(&self, name: &str, dims: usize, distance: Distance) -> SealStackResult<()> {
...
impl VectorStore for InMemoryStore {
    ...
    async fn upsert(&self, collection: &str, chunks: Vec<Chunk>) -> SealStackResult<()> {
        let mut entry = self.collections.get_mut(collection).ok_or_else(|| {
            SealStackError::NotFound(format!("collection `{collection}` not found"))
        })?;
...
mod tests {
    ...
    async fn dims_mismatch_is_validation_error() {
        ...
        assert!(matches!(err, SealStackError::Validation(_)));
    }
