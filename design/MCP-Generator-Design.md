...
1. **Compile-time.** `sealstack-csl` parses the CSL source, type-checks it, and emits a JSON manifest of MCP servers. Each schema becomes one server with a fixed toolset: `search_X`, `get_X`, `list_X`, one `list_X_<rel>` per `many` relation, and one `aggregate_X_<facet>` per `@facet` field.
...
3. **Runtime.** A POST to `/mcp/<server_name>` with a JSON-RPC body (`tools/list`, `tools/call`, etc.) hits the transport layer, which resolves the session, authenticates the caller, and dispatches to `protocol::dispatch`. For `tools/call`, that function looks up the handler in the registry and invokes it with the caller context attached. The handler validates arguments, calls into `sealstack-engine`, and packages the result into a `ToolsCallResult` with structured content.
...
### `sealstack-gateway/src/mcp/types.rs`
...
### `sealstack-gateway/src/mcp/registry.rs`
...
### `sealstack-gateway/src/mcp/protocol.rs`
...
### `sealstack-gateway/src/mcp/transport.rs`
...
### `sealstack-gateway/src/mcp/oauth.rs`
...
### `sealstack-gateway/src/mcp/handlers.rs`
...
2. **Stub engine only.** `StubEngine` returns `Unimplemented` for every call. Real implementations live in `sealstack-engine`. The gateway is ready for them.
...
```bash
cd /path/to/sealstack
cargo run --bin sealstack-gateway &
GATEWAY_PID=$!

# The registry is empty until you register handlers; add this as the boot step.
# In a test:
cargo test -p sealstack-gateway --test mcp_smoke
...
The `tools/list` call will return an empty array until you wire `sealstack-engine` and register generated handlers at boot. That wiring is the next piece of the scaffold — a ~50-line `register_schema` function that takes a `TypedFile` and a `ToolRegistry` and inserts handlers for every schema.
