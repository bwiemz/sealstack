# SealStack — Agent-Executable Scaffolding Brief

**Target repo**: `bwiemz/sealstack` (public, Apache-2.0)
...
**Completion criterion**: `sealstack dev` stands up the full stack in under 60 seconds on a clean machine, serves an MCP endpoint, and passes the end-to-end acceptance test in §12.
...
```
SealStack
Copyright 2026 SealStack Contributors

This product includes software developed by the SealStack project
(https://github.com/bwiemz/sealstack).
```
...
The SealStack name and logo are trademarks of [Entity]. The source code in
this repository is licensed under Apache-2.0, but that license does NOT grant
permission to use the SealStack name, logo, or branding. Forks and
...
```bash
for c in sealstack-common sealstack-csl sealstack-engine sealstack-gateway sealstack-ingest \
         sealstack-connector-sdk sealstack-embedders sealstack-vectorstore sealstack-receipts sealstack-cli; do
...
The `sealstack-cli` crate needs a `[[bin]]`:

```toml
# Append to crates/sealstack-cli/Cargo.toml
[[bin]]
name = "cfg"
path = "src/main.rs"
...
Create `crates/sealstack-cli/src/main.rs`:
```rust
use clap::Parser;

#[derive(Parser, Debug)]
#[command(name = "cfg", version, about = "SealStack CLI")]
enum Cmd {
...
**Verify**: `cargo build --bin cfg` succeeds and `./target/debug/sealstack version` prints `cfg 0.1.0`.

Commit: `git commit -am "feat(workspace): cargo workspace and cli skeleton"`.

---

## Phase 2 — Core Traits (sealstack-common, sealstack-vectorstore, sealstack-embedders, sealstack-connector-sdk)

These crates define the abstractions every other crate depends on. Nothing else should be built until these compile.

**`crates/sealstack-common/src/lib.rs`**
```rust
...
pub use error::{SealStackError, SealStackResult};
...
**`crates/sealstack-vectorstore/src/lib.rs`**
```rust
...
**`crates/sealstack-embedders/src/lib.rs`**
```rust
...
**`crates/sealstack-connector-sdk/src/lib.rs`**
```rust
...
pub trait Connector: Send + Sync + 'static {
    ...
    async fn list(&self) -> SealStackResult<Box<dyn tokio_stream::Stream<Item = Resource> + Send + Unpin>>;
    ...
    async fn subscribe(&self) -> SealStackResult<Option<Box<dyn tokio_stream::Stream<Item = ChangeEvent> + Send + Unpin>>> {
        Ok(None)
    }
}
...
Add `tokio-stream = "0.1"` to `sealstack-connector-sdk`.
...
**Files to create:**
- `crates/sealstack-csl/src/lib.rs`
- `crates/sealstack-csl/src/lexer.rs` (inline with parser, minimal tokens)
- `crates/sealstack-csl/src/parser.rs`  (uses `winnow`)
- `crates/sealstack-csl/src/ast.rs`
- `crates/sealstack-csl/src/types.rs`   (type checker)
- `crates/sealstack-csl/src/codegen/mod.rs`
- `crates/sealstack-csl/src/codegen/sql.rs`
- `crates/sealstack-csl/src/codegen/rust.rs`
- `crates/sealstack-csl/src/codegen/mcp.rs`
- `crates/sealstack-csl/src/codegen/vector_plan.rs`
- `crates/sealstack-csl/tests/parse_smoke.rs`
- `crates/sealstack-csl/tests/fixtures/hello.csl`
...
`crates/sealstack-csl/tests/parse_smoke.rs`:
```rust
use sealstack_csl::{compile, CompileTargets};
...
Run `cargo test -p sealstack-csl`, then `cargo insta review` and accept the snapshots. Commit the snapshot files.
...
**`crates/sealstack-engine/src/lib.rs`**
```rust
pub mod config;
pub mod memory;
pub mod retrieval;
...
Add `sqlx` migration directory `crates/sealstack-engine/migrations/20260101000000_init.sql`:

```sql
CREATE TABLE IF NOT EXISTS sealstack_schemas (
...
CREATE TABLE IF NOT EXISTS sealstack_connectors (
    id          ulid primary key,
    name        text not null,
...
CREATE TABLE IF NOT EXISTS sealstack_receipts (
    id          ulid primary key,
...
Run `cargo check -p sealstack-engine`.
...
**`crates/sealstack-gateway/src/lib.rs`**
```rust
pub mod server;
pub mod rest;
pub mod mcp;
...
`crates/sealstack-gateway/tests/health.rs`:
```rust
#[tokio::test]
async fn healthz_returns_200() {
    let app = sealstack_gateway::server::build_app(sealstack_gateway::config::Config::test()).await.unwrap();
...
**Verify**: `cargo test -p sealstack-gateway` passes.
...
Each is a separate crate that depends on `sealstack-connector-sdk`. Each implements the `Connector` trait and exposes a `register()` function that the ingest runtime uses to load it.
...
- `new SealStackClient({ baseUrl, token })`
- `.query(q, opts)` → `Promise<ContextResponse>`
- `.schemas.list()` / `.schemas.get(name)`
- `.connectors.list()` / `.connectors.sync(id)`

Publish config under `@sealstack/sdk` with `exports` for ESM + CJS.

### Python SDK

`sdks/python/sealstack/` — Pydantic v2 models + an `httpx` client. Mirrors the TS surface.
...
## Phase 8 — Dev Stack (Docker Compose) and CLI `sealstack dev`
...
services:
  postgres:
    ...
    ports: ["5432:5432"]
    volumes: [sealstack_pg:/var/lib/postgresql/data]
    healthcheck:
  ...
  qdrant:
    ...
    volumes: [sealstack_qdrant:/qdrant/storage]
    healthcheck:
  ...
  gateway:
    ...
    ports: ["7070:7070"]
    environment:
      SEALSTACK_DATABASE_URL: postgres://sealstack:sealstack@postgres:5432/sealstack
      SEALSTACK_QDRANT_URL: http://qdrant:6334
      SEALSTACK_REDIS_URL: redis://redis:6379
    depends_on:
...
COPY . .
RUN cargo build --release --bin sealstack-gateway
...
Add a binary target to `sealstack-gateway`:
```toml
[[bin]]
name = "sealstack-gateway"
path = "src/bin/server.rs"
```

Implement `sealstack dev` in `crates/sealstack-cli/src/cmd_dev.rs` to:
1. Locate the compose file (relative to the crate root or via `$SEALSTACK_HOME`).
...
**Verify**: from a clean machine (no running services), `sealstack dev` returns within 60 seconds with the ready banner, and `curl http://localhost:7070/healthz` returns 200.

Commit: `git commit -am "feat(devx): docker compose stack and sealstack dev command"`.
    ...
    steps:
      ...
      - run: cargo test --test e2e --locked
        env:
          ...
          SEALSTACK_QDRANT_URL:   http://localhost:6334
```
...
`crates/sealstack-gateway/tests/e2e.rs`:
```rust
...
async fn scaffold_end_to_end() -> anyhow::Result<()> {
    let env = sealstack_test::Env::spin_up().await?;
...
- [ ] `sealstack version` prints the expected version.
- [ ] `sealstack dev` starts the stack within 60 seconds.
- [ ] `sealstack schema apply`, `connector add`, `connector sync`, `query` all succeed against the running stack.
