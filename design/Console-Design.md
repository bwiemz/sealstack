...
The console is the web face of ContextForge for operators — engineering leads, infra teams, security auditors. It's a narrow SPA with seven meaningful views, not a sprawling admin app. It covers what the CLI covers, in shapes a human can read.
...
Below: two side-by-side list previews (recent schemas, recent connectors) that gracefully empty-state into quickstart CLI snippets. Empty states are operator-educational — a new user who lands on the dashboard with a running-but-empty gateway gets a literal `cfg schema apply` incantation to copy.
