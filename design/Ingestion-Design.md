...
**Companion to**: `crates/sealstack-connector-sdk/`, `crates/sealstack-ingest/`, and `connectors/local-files/` in the drop-in.
...
|---|---|---|
| `sealstack-connector-sdk` | Defines the `Connector` trait and the `Resource` / `ChangeEvent` shapes | `sealstack-common` |
| `sealstack-ingest` | Runtime that drives connectors and feeds their output into the engine | `sealstack-engine`, `sealstack-connector-sdk` |
| `connectors/local-files` | A concrete `Connector` implementation backed by a filesystem directory | `sealstack-connector-sdk` |
...
- The ingest runtime is an internal engine subsystem — nobody outside SealStack calls it directly.
...
## 2. `sealstack-connector-sdk` — the trait
...
pub trait Connector: Send + Sync + 'static {
    ...
    async fn subscribe(&self) -> SealStackResult<Option<ChangeStream>> { Ok(None) }
    async fn healthcheck(&self) -> SealStackResult<()> { Ok(()) }
}
```
...
## 3. `sealstack-ingest` — the runtime
...
1. **`sync_once(binding_id)`** — one-shot. The CLI's `sealstack connector sync <id>` and the integration-test harness both call this. Returns a `SyncOutcome` with counters for resources seen / ingested / failed plus elapsed time.
...
- **Incremental sync.** `sealstack_ingest_state` is created by the engine's migration but not read — every `sync_once` walks the full source. Idempotent upserts mean this is correct, just wasteful at scale.
...
`max_file_bytes` defaults to 2 MiB. Files larger than that are skipped with a `SealStackError::Validation` that the runtime logs as a warning. Two reasons:
...
async fn scaffold_end_to_end() -> anyhow::Result<()> {
    ...
    let connector = Arc::new(
        sealstack_connector_local_files::LocalFilesConnector::new(
...
1. **Incremental sync.** Write connector state (`last_sync_at`, `cursor`) to `sealstack_ingest_state` on every run. Skip unchanged resources on the next run. Significant efficiency win at scale.

2. **Delete propagation.** `ChangeEvent::Delete` is parsed but not acted on — the ingest runtime logs it and moves on. Needs a `(connector_resource_id, engine_record_id)` lookup in `sealstack_lineage` so we can delete the right row.
...
5. **Dead-letter queue.** `sealstack_ingest_dlq` table with `(connector_id, resource_id, last_error, attempts, next_retry_at)`. CLI command to replay.
...
9. **Binary file handling.** PDFs and Office docs are common in enterprise. A `sealstack-extract` crate with `pdf-extract` and `docx-rs` support, invoked from the connector before returning the `Resource`.
...
The `sealstack-connector-sdk` tests run without any external services. `sealstack-ingest` unit tests are registry-only and also run clean. `local-files` tests need a writable `tempfile::tempdir` but nothing else. Smoke-test those three first.
