...
```bash
mkdir contextforge && cd contextforge
git init -b main
```
...
The ContextForge name and logo are trademarks of [Entity]. The source code in
this repository is licensed under Apache-2.0, but that license does NOT grant
permission to use the ContextForge name, logo, or branding. Forks and
...
members = [
    ...
    "crates/cfg-gateway",
    "crates/cfg-ingest",
    "crates/cfg-connector-sdk",
    "crates/cfg-embedders",
    "crates/cfg-vectorstore",
    "crates/cfg-receipts",
    "crates/cfg-cli",
    "connectors/*",
]
default-members = ["crates/cfg-cli"]
...
```bash
for c in cfg-common cfg-csl cfg-engine cfg-gateway cfg-ingest \
         cfg-connector-sdk cfg-embedders cfg-vectorstore cfg-receipts cfg-cli; do
...
The `cfg-cli` crate needs a `[[bin]]`:

```toml
# Append to crates/cfg-cli/Cargo.toml
[[bin]]
name = "cfg"
path = "src/main.rs"
...
Create `crates/cfg-cli/src/main.rs`:
```rust
use clap::Parser;

#[derive(Parser, Debug)]
#[command(name = "cfg", version, about = "ContextForge CLI")]
enum Cmd {
...
**Verify**: `cargo build --bin cfg` succeeds and `./target/debug/cfg version` prints `cfg 0.1.0`.

Commit: `git commit -am "feat(workspace): cargo workspace and cli skeleton"`.

---

## Phase 2 — Core Traits (cfg-common, cfg-vectorstore, cfg-embedders, cfg-connector-sdk)

These crates define the abstractions every other crate depends on. Nothing else should be built until these compile.

**`crates/cfg-common/src/lib.rs`**
```rust
...
**`crates/cfg-vectorstore/src/lib.rs`**
```rust
...
pub trait VectorStore: Send + Sync + 'static {
    async fn ensure_collection(&self, name: &str, dims: usize) -> CfgResult<()>;
    async fn upsert(&self, collection: &str, chunks: Vec<Chunk>) -> CfgResult<()>;
    async fn search(
        &self,
        collection: &str,
        query_vec: Vec<f32>,
    ...
    ) -> CfgResult<Vec<SearchResult>>;
    async fn delete(&self, collection: &str, ids: Vec<ulid::Ulid>) -> CfgResult<()>;
}
...
**`crates/cfg-embedders/src/lib.rs`**
```rust
...
pub trait Embedder: Send + Sync + 'static {
    fn name(&self) -> &str;
    fn dims(&self) -> usize;
    async fn embed(&self, texts: Vec<String>) -> CfgResult<Vec<Vec<f32>>>;
}
...
**`crates/cfg-connector-sdk/src/lib.rs`**
```rust
...
pub trait Connector: Send + Sync + 'static {
    ...
    async fn list(&self) -> CfgResult<Box<dyn tokio_stream::Stream<Item = Resource> + Send + Unpin>>;
    ...
    async fn subscribe(&self) -> CfgResult<Option<Box<dyn tokio_stream::Stream<Item = ChangeEvent> + Send + Unpin>>> {
        Ok(None)
    }
}
...
Add `tokio-stream = "0.1"` to `cfg-connector-sdk`.
...
**Files to create:**
- `crates/cfg-csl/src/lib.rs`
- `crates/cfg-csl/src/lexer.rs` (inline with parser, minimal tokens)
- `crates/cfg-csl/src/parser.rs`  (uses `winnow`)
- `crates/cfg-csl/src/ast.rs`
- `crates/cfg-csl/src/types.rs`   (type checker)
- `crates/cfg-csl/src/codegen/mod.rs`
- `crates/cfg-csl/src/codegen/sql.rs`
- `crates/cfg-csl/src/codegen/rust.rs`
- `crates/cfg-csl/src/codegen/mcp.rs`
- `crates/cfg-csl/src/codegen/vector_plan.rs`
- `crates/cfg-csl/tests/parse_smoke.rs`
- `crates/cfg-csl/tests/fixtures/hello.csl`
...
`crates/cfg-csl/tests/parse_smoke.rs`:
```rust
use cfg_csl::{compile, CompileTargets};
...
Run `cargo test -p cfg-csl`, then `cargo insta review` and accept the snapshots. Commit the snapshot files.
...
Add `sqlx` migration directory `crates/cfg-engine/migrations/20260101000000_init.sql`:

```sql
CREATE TABLE IF NOT EXISTS cfg_schemas (
...
CREATE TABLE IF NOT EXISTS cfg_connectors (
    id          ulid primary key,
    name        text not null,
...
CREATE TABLE IF NOT EXISTS cfg_receipts (
    id          ulid primary key,
...
`crates/cfg-gateway/tests/health.rs`:
```rust
#[tokio::test]
async fn healthz_returns_200() {
    let app = cfg_gateway::server::build_app(cfg_gateway::config::Config::test()).await.unwrap();
...
**Verify**: `cargo test -p cfg-gateway` passes.
...
Each is a separate crate that depends on `cfg-connector-sdk`. Each implements the `Connector` trait and exposes a `register()` function that the ingest runtime uses to load it.
...
- `new ContextForgeClient({ baseUrl, token })`
- `.query(q, opts)` → `Promise<ContextResponse>`
- `.schemas.list()` / `.schemas.get(name)`
- `.connectors.list()` / `.connectors.sync(id)`

Publish config under `@contextforge/sdk` with `exports` for ESM + CJS.

### Python SDK

`sdks/python/contextforge/` — Pydantic v2 models + an `httpx` client. Mirrors the TS surface.
...
## Phase 8 — Dev Stack (Docker Compose) and CLI `cfg dev`
...
services:
  postgres:
    ...
    environment:
      POSTGRES_PASSWORD: cfg
      POSTGRES_USER: cfg
      POSTGRES_DB: cfg
    ports: ["5432:5432"]
    volumes: [cfg_pg:/var/lib/postgresql/data]
    healthcheck:
  ...
  qdrant:
    ...
    volumes: [cfg_qdrant:/qdrant/storage]
    healthcheck:
  ...
  gateway:
    ...
    ports: ["7070:7070"]
    environment:
      CFG_DATABASE_URL: postgres://cfg:cfg@postgres:5432/cfg
      CFG_QDRANT_URL: http://qdrant:6334
...
COPY . .
RUN cargo build --release --bin cfg-gateway

FROM gcr.io/distroless/cc-debian12
COPY --from=builder /build/target/release/cfg-gateway /usr/local/bin/cfg-gateway
EXPOSE 7070
ENTRYPOINT ["/usr/local/bin/cfg-gateway"]
```

Add a binary target to `cfg-gateway`:
```toml
[[bin]]
name = "cfg-gateway"
...
Implement `cfg dev` in `crates/cfg-cli/src/cmd_dev.rs` to:
1. Locate the compose file (relative to the crate root or via `$CFG_HOME`).
...
**Verify**: from a clean machine (no running services), `cfg dev` returns within 60 seconds with the ready banner, and `curl http://localhost:7070/healthz` returns 200.

Commit: `git commit -am "feat(devx): docker compose stack and cfg dev command"`.
    ...
    steps:
      ...
      - run: cargo test --test e2e --locked
        env:
          ...
          CFG_QDRANT_URL:   http://localhost:6334
```
...
`crates/cfg-gateway/tests/e2e.rs`:
```rust
...
- [ ] `cfg version` prints the expected version.
- [ ] `cfg dev` starts the stack within 60 seconds.
- [ ] `cfg schema apply`, `connector add`, `connector sync`, `query` all succeed against the running stack.
