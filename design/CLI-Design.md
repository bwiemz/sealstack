...
**Companion to**: `crates/sealstack-cli/` in the drop-in, plus the updated `crates/sealstack-gateway/src/{rest,server,bin/server}.rs`.
**Status**: Unverified skeleton. 1,539 LOC for the CLI, ~300 LOC of gateway REST handlers, plus a small patch to `sealstack-csl` that emits schema metadata for the CLI to forward.
...
1. **A thin HTTP client** over the gateway's REST surface. `sealstack schema list`, `sealstack connector sync`, `sealstack query`, `sealstack receipt` — all of them just hit an endpoint and render the response.
2. **A local CSL compiler driver.** `sealstack compile` and `sealstack schema validate` run `sealstack_csl::compile()` in-process without touching the gateway. `sealstack schema apply` is the hybrid — compile locally, then POST the results.
3. **A Docker Compose wrapper.** `sealstack dev` shells out to `docker compose up -d` against a well-known file path, then polls `/healthz` until ready.
...
```
  cfg <subcommand> ────▶ cli::Cli::parse
                       ...
                       │             │
                       ▼             ▼
                 client::Client   sealstack_csl::compile  (+ filesystem I/O)
                 (HTTP client)
                       │
                       ▼
...
`sealstack schema apply <path>` is the only command that splits work between the CLI and the gateway:

1. CLI reads the `.csl` file from disk.
2. CLI calls `sealstack_csl::compile(src, CompileTargets::all())`.
...
Two round trips, not one. The split matters because the DDL is persisted in Postgres and survives a gateway restart, while the in-memory registry does not — the next restart will re-hydrate from `sealstack_schemas` if we wire that up, but today the operator re-runs `sealstack schema apply` on restart.
...
### The sealstack-csl patch

To make `schema apply` work, I patched `sealstack-csl` to emit a `schemas_meta: Vec<Value>` field on `CompileOutput`. Each entry is a JSON document matching `sealstack_engine::schema_registry::SchemaMeta` — namespace, name, version, primary key, fields with decorator flags (`primary`, `indexed`, `searchable`, `chunked`, `facet`, `unique`, `optional`, plus `boost` and `pii` values), relations, context block, collection name (`<table>_v<version>`), table name (snake_cased schema name), and optional schema-level hybrid alpha.
...
**Caveat:** the emitter makes strong assumptions about the `sealstack-csl` AST shape — specifically `FieldDecl.decorators`, `Decorator::is()`, `Arg.name`, `Expr::Call`, `Literal::Duration`, and `rel.kind.as_str()`. If any of those differ from what my AST module declares, the patch will fail to compile. The fix is local and mechanical — adjust the field accesses to match the actual AST.
...
The gateway's `CallerExt` extractor picks these up and constructs a `sealstack_engine::api::Caller` that flows through the engine's policy layer.
...
5. **`sealstack-csl` emits `schemas_meta`** on `CompileOutput`. See §4 above.

---

## 8. Known gaps — priority order

1. **Compile-error rendering.** `sealstack schema validate` currently prints `CslError::Display`. The CSL spec calls for `miette` rendering with spans, underlines, and help text. Adding `miette` as a CLI dep and a one-liner `Report::new(err).with_source_code(src)` gives the full experience.

2. **Real auth.** Header-based identity is a dev convenience. Wire in JWT validation in a Tower middleware layer ahead of `CallerExt`. Operators get an `OAUTH_TOKEN` or `SEALSTACK_TOKEN` env var; the CLI sends it as a bearer. Target: next sprint.

3. **Receipt retrieval latency.** `sealstack receipt <id>` hits `/v1/receipts/<id>`, which does a single `SELECT` against `sealstack_receipts`. Fast, but there's no caching. Not a problem yet.

4. **No interactive confirmation.** `sealstack schema apply` on an existing schema silently overwrites the in-memory registry entry. A real production flow should prompt unless `--yes` is passed.

5. **`cfg watch` is missing.** The local-files connector already supports `subscribe`. The CLI should expose `sealstack connector watch <id>` that spawns a long-running session tailing change events. ~50 LOC.

6. **Background connector scheduling.** `sealstack connector add` registers a binding with `interval = None`; the gateway's `IngestRuntime::start_background` is never called. For operational deployments we want a `--interval 5m` flag plus a `sealstack connector schedule` command.

7. **`sealstack dev --tail-logs`.** After `docker compose up -d`, many operators immediately run `docker compose logs -f`. A `--tail-logs` flag should just do that inline.
...
10. **Rate-limit the `healthz` poll in `sealstack dev`.** Currently 500 ms. Should back off exponentially to avoid log spam when the gateway takes 30+ seconds to come up on a fresh docker build.
...
6. **clap `env = "SEALSTACK_GATEWAY_URL"` + `default_value = "..."`.** Stable since clap 4.0.

7. **`toml` 0.8 deserializer.** Works through `toml::from_str`. Stable.

If `cargo check -p sealstack-cli` passes with the CLI alone — which doesn't depend on `sealstack-engine`, `sealstack-gateway`, or the connector SDK — the bulk of the code is valid. The gateway-side patches are the higher-risk surface.
...
sealstack connector add local-files --schema examples.Doc --root ./sample-docs
sealstack connector sync local-files/examples.Doc
result=$(sealstack query "getting started" --schema examples.Doc --json)
echo "$result" | jq -e '.results | length > 0'
receipt=$(echo "$result" | jq -r .receipt_id)
sealstack receipt "$receipt" --json | jq -e '.sources | length > 0'
```
