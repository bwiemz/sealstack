...
**SealStack** is the third thing: an open-core *context operating system* — infrastructure that unifies ingestion, retrieval, memory, policy, and tool-use under a single typed context model, exposes every internal system as a governed MCP server, and ships both a free self-hostable core and a managed commercial tier. It is purpose-built for the 2026 reality where every AI system speaks MCP and every company needs the same five-layer context stack.
...
| Platform | Layer(s) | Model | Pricing | Gap SealStack exploits |
|---|---|---|---|---|
...
**The wedge:** there is no opinionated, open-core, MCP-native, self-hostable *platform* that covers all five context layers (ingestion, memory, retrieval, policy, tooling) with first-class developer DX and typed context schemas. That is the slot SealStack fills.
...
2. **MCP-native.** SealStack is an MCP host *and* auto-generates MCP servers from connectors, schemas, and policies. Any AI client (Claude, ChatGPT, Cursor, Gemini, internal agents) can consume context without custom integration.
...
- Connector SDK (`sealstack-connector-sdk` in Rust and TypeScript) with a 200-line minimum contract: `list()`, `fetch()`, `subscribe()`, `map_permissions()`.
...
- **Lineage**: every chunk carries a full provenance graph from source → parse → chunk → embed → retrieve. Every answer SealStack serves can produce a signed receipt of which sources contributed, which policies applied, and which caller asked.
...
Every response produced through SealStack ships with a verifiable receipt: the caller identity, the policies that applied, the sources retrieved (with versions and timestamps), and the LLM's final answer. Receipts are content-addressed and optionally signed. This is the audit-trail/compliance story that enterprise security teams are actively buying.

### 5.4 Local-first mode

`sealstack dev` stands up the entire stack on a laptop with Docker Compose — Postgres, Qdrant, the engine, and the console — in under 60 seconds. No cloud dependency. Drop an API key in, point at a GitHub repo and a Slack workspace, and you have a working context layer the same afternoon. This is the on-ramp the closed incumbents structurally cannot offer.

### 5.5 Shadow mode for migration

Import existing RAG pipelines (LangChain, LlamaIndex, raw Pinecone collections) and run SealStack alongside them in "shadow mode" — it replays queries, compares retrieval quality, and emits a delta report. This turns the migration conversation from a full-platform rewrite into an A/B evaluation.
...
- `sealstack dev` single-command local bring-up via Docker Compose
...
Transparent pricing is one of the top three buyer complaints about Glean — every comparison article leads with it. SealStack ships public pricing from day one.
...
- **Why Apache-2.0 (not MIT)**: patent grant clause. SealStack sits adjacent to prior art in search, retrieval, and policy evaluation — explicit patent grants protect contributors and users. MIT doesn't. Dust/Onyx picked MIT; we can differentiate on this.
...
### Private repo — `bwiemz/sealstack-enterprise`

- **License**: SealStack Commercial License (EULA) — template derived from your QuantForge EULA
- **Contents**:
...
- Register "SealStack" wordmark and logo (class 9 software, class 42 SaaS).
...
│
├── crates/
│   ├── sealstack-engine/                   # core runtime (L2+L3+L4)
│   │   ├── src/
...
│   │   ├── tests/
│   │   └── Cargo.toml
│   ├── sealstack-gateway/                  # L5: HTTP + MCP surface
│   │   ├── src/
│   │   │   ├── mcp/                  # MCP server impl
│   │   │   ├── rest/                 # REST endpoints
...
│   │   │   └── lib.rs
│   │   └── Cargo.toml
│   ├── sealstack-connector-sdk/            # Rust connector trait + helpers
│   ├── sealstack-embedders/                # embedder adapters
│   ├── sealstack-vectorstore/              # vector store trait + adapters
│   ├── sealstack-cli/                      # `cfg` binary
│   ├── sealstack-common/                   # shared types, errors, telemetry
│   └── sealstack-receipts/                 # grounded answer receipts
│
├── connectors/
...
│
├── sdks/
│   ├── typescript/                   # @sealstack/sdk
│   │   ├── src/
│   │   ├── package.json
│   │   └── tsconfig.json
│   ├── python/                       # sealstack (PyPI)
│   │   ├── sealstack/
│   │   ├── pyproject.toml
│   │   └── tests/
│   └── go/                           # github.com/sealstack/go-sdk
│
...
│   │   ├── Dockerfile.gateway
│   │   ├── Dockerfile.ingest
│   │   └── compose.dev.yaml          # `sealstack dev` stack
│   ├── helm/
│   │   └── sealstack/
│   │       ├── Chart.yaml
│   │       ├── values.yaml
...
```
sealstack-enterprise/
├── .github/
│   └── workflows/
│       ├── ci.yml
│       └── release.yml
├── crates/
│   ├── sealstack-cluster/                  # horizontal scaling, consensus
│   ├── sealstack-premium-policy/           # advanced governance
│   ├── sealstack-sso-extensions/           # SCIM, advanced SAML
│   ├── sealstack-compliance/               # FedRAMP, HIPAA, ISO pack generators
│   └── sealstack-pii/                      # advanced PII/DLP
...
- `sealstack dev` single-command local bring-up
- CLI MVP (`sealstack schema`, `sealstack connector`, `sealstack query`)
